...
# ---------------------------------------------------------------------------
# In-memory baseline store as a placeholder until Tier 0 (real DB) lands.
...
# ---------------------------------------------------------------------------
_SESSION_SAMPLES: dict[str, list[dict]] = {}
...
async def analyze_voice_chunk(
    ...
    baseline = _compute_baseline(user_id)
    is_spike = False
    spike_reason = None
    ...
    return VoiceBiomarkerResponse(
        **result,
        ...
        session_sample_count=len(_SESSION_SAMPLES.get(session_id, [])),
    )


def _compute_baseline(user_id: str) -> dict:
    """
    Placeholder baseline computation across all of this user's stored
    sessions. TODO once Tier 0 DB lands: replace _SESSION_SAMPLES lookup
    with a real query grouped by user_id across historical sessions
    (currently this only sees samples from the current process's memory).
    """
    all_samples = [
        s for samples in _SESSION_SAMPLES.values() for s in samples
        if s.get("jitter_pct") is not None
    ]
